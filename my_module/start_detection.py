...
def for_bin_detect(AE_type,num_epochs,optimizer_type,learning_rate,weight_decay,input_size,train_csv_path,test_csv_path,train_image_path,test_image_path,test_img_index,train_name,error_th):
    ...
    loss_list, model = cal.calculate(num_epochs,train_loader,model,optimizer,
            f"model/train_folder={train_name} image_type={save_type} optim={optimizer_type} lr={learning_rate} epoch={num_epochs}.pkl") 
    np.save(f"./model/loss_list.npy", np.array(loss_list)) 
...
def for_expert_detect(AE_type,num_epochs,optimizer_type,learning_rate,weight_decay,
    ...
    train_loader_color, test_loader_color = mds.color_make_data_set(
       train_csv_path,test_csv_path,train_image_path,test_image_path) 


    train_loader_lab, test_loader_lab = mds.lab_make_data_set(
       train_csv_path,test_csv_path,train_image_path,test_image_path) 
    ...
    if error_th == "color":
        ...
        save_type = "color"
        loss_list, model = cal.calculate(num_epochs,train_loader_color,model,optimizer,
                f"model/train_folder={train_name} image_type={save_type} AE_type={AE_type} optim={optimizer_type} lr={learning_rate} epoch={num_epochs}.pkl",AE_type) 
    else:
        
        loss_list, model = cal.calculate(num_epochs,train_loader_lab,model,optimizer,
                f"model/train_folder={train_name} image_type={save_type} AE_type={AE_type} optim={optimizer_type} lr={learning_rate} epoch={num_epochs}.pkl",AE_type) 
